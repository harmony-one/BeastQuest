...
contract BQSale is FixedSupplyLotSale {
     ...
     */
    function updateNonFungibleSupply(
        uint256[] memory nonFungibleTokens,
        uint256 supplyOffset
    ) public onlyOwner // whenNotStarted
    {
        require(nonFungibleTokens.length > 0);

        uint256 newSupplySize = _nonFungibleSupply.length.add(
        ...
        returns (uint256[] memory)
    {
        ...
        bytes32 _seed = blockhash(block.number - 1);
